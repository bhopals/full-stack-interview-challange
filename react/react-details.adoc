...
*** Using React.createClass will automatically bind this context (values) correctly, but that is not the case when using
ES6 classes (React.Component)
...
For marking Ajax Request, we can use  axios, SuperAgent, or even plain Javascript).
...
be able to measure it. ReactJS provides with addon tools to measure performance. 
Import the react-addons-perf module to measure the performance
