...
kubectl create -f "<yaml>.yaml"
kubectl apply -f <git-url>
...
===== Scaling the APP 
    ...
    kubectl delete pods -l env=staging -n <name-space>
        - Delete all the pods that has  env = staging and belong to given namespace.


=== Make it Production Ready 

==== Minikube 
minikube implements a local Kubernetes cluster on macOS, Linux, and Windows. minikube's primary goals are to be
the best tool for local Kubernetes application development and to support all Kubernetes features that fit.

==== Kubeadm
Kubeadm is a tool built to provide kubeadm init and kubeadm join as best-practice “fast paths” for creating Kubernetes clusters.

==== kops (Kuberenetes Operations)
...
supported, with GCE in beta support , and VMware vSphere in alpha, and other platforms planned.

==== Kompose 
Kompose tool converts Docker Compose files into Kubernetes 
Object like deployments and Services.

==== Helm 
Helm is a tool that streamlines the installation and management 
of Kubernetes applications. It is like a package manager for kubernenetes.
